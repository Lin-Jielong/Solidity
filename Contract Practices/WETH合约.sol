...
// WETH 是包装 ETH 的主币，作为 ERC20 的合约。标准的 ERC20 合约包括如下几个

// 3个查询
//   - balanceOf(address) ：查询指定地址的Token数量
//   - totalSupply() ：查询当前合约的Token总量
//   - allowance(address, address) ：查询指定地址对另一地址的剩余授权额度
// 2个交易
//   - transfer(address, uint256) ：从当前调用者发送指定数量的Token到指定地址
//     - 这是一个写入方法，所以还会抛出一个 Transfer 事件
//   - transferFrom(address, address, uint256) ：当向另一个合约地址存款时，
//     对方合约必须调用 transferFrom 才可以把 Token 拿到它自己的合约中。
